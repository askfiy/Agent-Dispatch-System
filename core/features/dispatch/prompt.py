...
## 角色
计划更新员。你的唯一目标是将用户的反馈信息，作为一条记录，追加到 `Process.md` 中对应的步骤下方。**你只做追加, 不要修改其他任何原文**.

## 步骤
1.  **定位步骤**:
    在 `Process.md` 中，找到那个等待用户输入的步骤。这通常是第一个未完成 `[ ]` 的步骤，其目标与我们向用户提出的问题 (`notify_user`) 相关。

2.  **更新计划**:
    对定位到的步骤执行以下两项操作：
    a. 将其状态从 `[ ]` 修改为 `[x]`。
    b. 在该步骤的描述下方，新增一行 **若有 Output, 则不要删除已有的 Output**，而是将其更新在 **Output** 的正下方.
    c. 格式必须为: `> **Input**: user_message`。其中 `user_message` 是用户的完整回复。
...
2.  **更新计划**:
    a. 我将 `#2` 的 `[ ]` 改为 `[x]`。
    b. 我将在 `#2` 的 `Output` 下方添加新的一行： `> **Input**: 内容很棒，我们就深入探讨事件循环机制这个方向吧。`
3. **最终结果**
    最终结果应该变为:
    - [x] #2 用户选择具体方向
      > Objective: 在多个子主题中，用户将选择一个具体方向进行深入撰写。
...
## 输出格式要求
你的输出格式必须是 JSON 格式。`process` 字段必须是你修改后的、最新的完整 `Process.md`, 注意: **只做增量操作, 不要大改原文**。
...
def task_run_next_prompt(
    output_cls: type[LLMOutputModel], unit_content: list[dict[str, Any]]
):
...
当任务状态因需要用户审核而进入 `WAITING` 状态时，你的核心职责是**完整地、一字不差地展示**需要被审核的产物（Artifact），而不是对它进行总结或生成一个引导问题。

用户必须看到原始、完整的文本才能做出正确的决策。

## 角色

高级任务规划专家。你必须严格遵循黄金法则和所有步骤。

对于 Process.md 而言, **你只做追加, 不要修改其他任何原文**.
...
### 2. 更新 Process
- 在 `Process.md` 中找到对应的执行单元。
- 将其状态标记为 `- [x]` (成功/待审) 或 `- [!]` (失败)。
- **关键**: 在该单元的 `> **Output**:` 字段后，为产出生成一个**不超过 50 字的简洁摘要**。摘要应抓住核心结果，而非全文复制。

### 3. 决策
综合分析全局，按以下唯一优先级顺序，为任务决定下一个状态。满足其一即停止。

1.  **`WAITING`**: 如果任一 `output` 是需要用户审查的草稿、向用户提问、或提供选项。
    - **应用黄金法则**: 此刻，你的唯一目标是展示产物。
    - **核心指令 (必须严格执行)**:
        1.  **定位产物**: 找到那个触发了 `WAITING` 状态的执行单元的完整 `output`。
        2.  **禁止总结**: 严禁以任何形式对该 `output` 进行总结、缩写、或生成“请查看...”之类的引导语。
        3.  **逐字复制**: 将该 `output` 的**全部内容**，一字不差地、原封不动地复制到 `notify_user` 字段中。
        4.  **追加问题**: 在**复制的全文内容之后**，另起一行，附上引导用户下一步操作的问题（例如：“请选择一个具体方向进行详细撰写。”）。

2.  **处理失败**: 如果任一单元被标记为 `[!]` (失败)。
    - **指令**: 按以下策略解决此失败：
        - a. **诊断**: 分析错误是临时性还是永久性。
        - b. **制定解决方案**: 若为永久性错误，**必须修改 `Process.md`**，设计替代方案。
        - c. **无法解决**: 若无法制定新方案，将状态设为 `WAITING`，并在 `notify_user` 中解释问题并请求帮助。
    - **决策**: 成功制定新计划后，任务状态为 `ACTIVATING`。

3.  **`FINISHED`**: `Process.md` 中所有单元均已标记 `[x]` 且无失败。

4.  **`ACTIVATING`**: `Process.md` 中存在可执行的后续步骤。

5.  **`FAILED`**: 经过失败处理后仍无法解决问题且无后续步骤（死锁）。
...
- `已执行的执行单元信息`:
    - 单元名称: `列出博客大纲`
    - 单元产出 (output): `### Python AsyncIO 大纲\n\n#### 一、简介...` (完整的 AsyncIO 大纲)

**THINKING**:
1.  **分析**: 收到 Process 和 #1 的产出。
2.  **更新**: 将 #1 标记为 `[x]` 并追加其 `output`。
3.  **决策**:
    - 检查 WAITING 条件: **满足**。因为 #2 步骤需要用户选择，这依赖于 #1 的产出。
    - **应用黄金法则**: 我的职责是展示 #1 的完整产物，而不是总结它。
    - **执行核心指令**:
        1.  **定位产物**: #1 的完整 `output`，也就是那个 Markdown 格式的大纲。
        2.  **禁止总结**: 我不会自己生成 "请查看以上大纲..." 这样的句子。
        3.  **逐字复制**: 我将把整个大纲的 Markdown 文本复制到 `notify_user` 字段。
        4.  **追加问题**: 复制结束后，我会在文本末尾加上问题，例如：“请选择一个具体方向进行详细撰写。”
    - 最终状态判定为 `WAITING`。
---
...
def task_run_unit_prompt(
    output_cls: type[LLMOutputModel], unit_content: list[dict[str, Any],], prd: str
):
    utc_now = datetime.datetime.now(datetime.timezone.utc)
    formatted = utc_now.strftime("%Y-%m-%d %H:%M:%S")
