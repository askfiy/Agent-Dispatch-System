...
class Tasks(BaseTableScheme):
    ...
    session_id: Mapped[str] = mapped_column(
        ...
        nullable=False,
        index=True,
        comment="任务建立时流转的会话 Id.",
    )

    prev_round_id: Mapped[uuid.UUID] = mapped_column(
        sa.CHAR(36), nullable=True, index=True, comment="上次任务的调用轮次 Id"
    )

    curr_round_id: Mapped[uuid.UUID] = mapped_column(
        sa.CHAR(36), nullable=True, index=True, comment="本次任务的调用轮次 Id"
    )
